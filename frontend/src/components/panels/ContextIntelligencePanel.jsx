...
export default function ContextIntelligencePanel({
  ...
  // National Overview Roster
  if (isNationalOverview || (!event && !source && !facility)) {
    return (
      <div className="w-full sm:w-[500px] bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 h-full flex flex-col shadow-2xl z-30 animate-in slide-in-from-right duration-200 text-slate-900 dark:text-slate-100 transition-colors">
        {/* Panel Header */}
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400 font-semibold">
                {facilities.length} PLANTS ACTIVE
              </span>
            </div>
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400">
              Select any facility marker on the map or click below to inspect thermal telemetry.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2.5 custom-scrollbar">
          <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider px-1">
            Configured National Industrial Complexes ({facilities.length})
          </div>
          {facilities.map((fac) => {
            ...
            return (
              <div
                ...
                className="p-3 rounded-lg border border-slate-200 dark:border-slate-800 hover:border-blue-500 dark:hover:border-blue-500 bg-white dark:bg-slate-800/60 hover:bg-blue-50/50 dark:hover:bg-blue-950/30 transition cursor-pointer space-y-1.5 shadow-2xs"
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="text-xs font-bold text-slate-900 dark:text-slate-100 hover:text-blue-600">
                      {fac.name}
                    </span>
                    <div className="text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-2">
                      <span>{fac.location || fac.state || 'India'}</span>
                      <span>•</span>
                      <span>{fac.industry_type || fac.industry || 'Petrochemical / Refinery'}</span>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400 font-mono pt-1 border-t border-slate-100 dark:border-slate-800/80">
                  <span>GPS: {fac.coordinates ? `${fac.coordinates[0].toFixed(3)}°N, ${fac.coordinates[1].toFixed(3)}°E` : 'N/A'}</span>
                  <span className="text-blue-600 dark:text-blue-400 font-sans font-semibold flex items-center gap-0.5">
                    Inspect &rarr;
                  </span>
                </div>
              </div>
  ...
  const isFacilityAbnormal = facility?.current_status !== 'NOMINAL_OPERATIONS' || (facility?.current_abnormality_score && facility?.current_abnormality_score > 40);

  // Facility coordinates
  const facCoords = facility?.coordinates || [21.6850, 72.5750];
  const evtCoords = event ? [event.latitude, event.longitude] : facCoords;

  // Exact Haversine Geodesic Distance from Facility to Active Event
  const spatialMetrics = useMemo(() => {
    if (!event || !facCoords || !evtCoords) {
      return { distanceKm: 0, distanceM: 0, bearingDeg: 0, compassDir: 'N/A' };
    }
    return calculateDistanceAndBearing(facCoords[0], facCoords[1], evtCoords[0], evtCoords[1]);
  }, [facCoords, evtCoords, event]);

  const fenceRadiusM = facility?.fence_radius_m || 450;
  const isInsideFence = spatialMetrics.distanceM <= fenceRadiusM;

  // Nearby Hotspots for this Facility with exact distance
  const nearbyHotspotsWithDistances = useMemo(() => {
    if (!thermalEvents || thermalEvents.length === 0) return [];
    return thermalEvents.map(evt => {
  ...
  }, [thermalEvents, facCoords, fenceRadiusM]);
  ...
  return (
    <div className="w-full sm:w-[500px] bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 h-full flex flex-col shadow-2xl z-30 animate-in slide-in-from-right duration-200 text-slate-900 dark:text-slate-100 transition-colors">
      ...
      <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex items-start justify-between gap-3 shrink-0">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${isFacilityAbnormal || isFire
      ...
      {/* Concise Operational Summary Header */}
      <div className="px-4 py-2.5 bg-slate-100/80 dark:bg-slate-800/40 border-b border-slate-200 dark:border-slate-800 text-[11px] space-y-1 shrink-0">
        <div className="grid grid-cols-2 gap-2">
          <div className="flex items-center justify-between bg-white dark:bg-slate-900 px-2 py-1 rounded border border-slate-200 dark:border-slate-700">
            <span className="text-slate-500 dark:text-slate-400 font-medium">Thermal State:</span>
          ...
          <div className="flex items-center justify-between bg-white dark:bg-slate-900 px-2 py-1 rounded border border-slate-200 dark:border-slate-700">
            <span className="text-slate-500 dark:text-slate-400 font-medium">Active Hotspots:</span>
            <span className="font-bold font-mono text-slate-900 dark:text-slate-100">{nearbyHotspotsWithDistances.length}</span>
          </div>
        </div>
        <div className="flex items-center justify-between bg-white dark:bg-slate-900 px-2 py-1 rounded border border-slate-200 dark:border-slate-700 text-[10px]">
          <span className="text-slate-500 dark:text-slate-400 font-medium">Industrial Telemetry:</span>
          <span className="font-mono text-slate-700 dark:text-slate-300 font-bold">STANDBY — AWAITING VERIFIED TELEMETRY</span>
        </div>
      </div>

      {/* Navigation Tabs */}
      <div className="flex border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 overflow-x-auto shrink-0">
        <button
          onClick={() => setActiveTab('overview')}
          className={`py-2 px-3 text-xs font-bold border-b-2 cursor-pointer transition-all whitespace-nowrap ${activeTab === 'overview' ? 'border-blue-600 text-blue-700 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'
            }`}
        >
          1. Overview
        </button>
        <button
          ...
          className={`py-2 px-3 text-xs font-bold border-b-2 cursor-pointer transition-all whitespace-nowrap ${activeTab === 'classification' ? 'border-blue-600 text-blue-700 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'
            }`}
        >
          2. 7-Class AI
        </button>
        <button
          ...
          className={`py-2 px-3 text-xs font-bold border-b-2 cursor-pointer transition-all whitespace-nowrap ${activeTab === 'evidence' ? 'border-blue-600 text-blue-700 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'
            }`}
        >
          3. Evidence & Fusion
        </button>
        <button
          ...
          className={`py-2 px-3 text-xs font-bold border-b-2 cursor-pointer transition-all whitespace-nowrap ${activeTab === 'domino' ? 'border-blue-600 text-blue-700 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'
            }`}
        >
          4. Cascade Risk
        </button>
        <button
          ...
          className={`py-2 px-3 text-xs font-bold border-b-2 cursor-pointer transition-all whitespace-nowrap ${activeTab === 'response' ? 'border-blue-600 text-blue-700 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'
            }`}
        >
          5. Response & SOS
        </button>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-3 animate-in fade-in duration-100">
            ...
            {/* Active Selected Hotspot Banner */}
            {event && (
              <div className="p-3 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-blue-900 dark:text-blue-200 flex items-center gap-1.5">
                    <Flame className="w-4 h-4 text-red-500" />
                    Selected Hotspot: <span className="font-mono">{event.event_id}</span>
                  </span>
                ...
                <div className="text-[11px] text-slate-700 dark:text-slate-300 font-medium">
                  This hotspot is <b className="text-blue-700 dark:text-blue-400 font-mono">{spatialMetrics.distanceKm < 1 ? `${Math.round(spatialMetrics.distanceM)}m` : `${spatialMetrics.distanceKm.toFixed(2)} km`}</b> ({spatialMetrics.compassDir}, {spatialMetrics.bearingDeg}°) from <span className="font-semibold">{facility?.name || 'Facility'}</span>.
                </div>
              </div>
            )}
            ...
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2 text-[11px] text-center">
                <div className="bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-200 dark:border-slate-700">
                  <span className="text-slate-400 block text-[10px]">FRP BASELINE</span>
                  <span className="font-mono font-bold text-slate-900 dark:text-slate-100">12.4 MW</span>
                </div>
                <div className="bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-200 dark:border-slate-700">
                  <span className="text-slate-400 block text-[10px]">TEMP BASELINE</span>
                  <span className="font-mono font-bold text-slate-900 dark:text-slate-100">318 K</span>
                </div>
                <div className="bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-200 dark:border-slate-700">
                  <span className="text-slate-400 block text-[10px]">Z_FRP SPIKE</span>
                  <span className="font-mono font-bold text-red-600 dark:text-red-400">+4.82 σ</span>
                </div>
              </div>
            </div>

            {/* Nearby Hotspots Roster with Geodesic Distance */}
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-2">
              <div className="font-bold text-slate-800 dark:text-slate-200 flex justify-between items-center">
                <span>Nearby Thermal Sources around Facility ({nearbyHotspotsWithDistances.length})</span>
                <span className="text-[10px] text-blue-600 dark:text-blue-400 font-bold">Click to Inspect</span>
              </div>
              ...
              {nearbyHotspotsWithDistances.length > 0 ? (
                ...
                <div className="text-xs text-slate-500 p-2 text-center">No active thermal hotspots within 50km radius.</div>
              )}
            </div>
          </div>
        )}
            ...
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-2">
              ...
              {showFeaturesContract && (
                <div className="grid grid-cols-2 gap-1.5 text-[10px] font-mono pt-1 text-slate-600 dark:text-slate-300">
                  ...
                  <div className="bg-white dark:bg-slate-900 p-1.5 rounded border border-slate-200 dark:border-slate-700">frp_robust_zscore: +4.82</div>
